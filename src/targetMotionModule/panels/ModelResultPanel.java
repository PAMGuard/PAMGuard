...
public class ModelResultPanel implements TMDialogComponent {
	...
	public ModelResultPanel(TargetMotionLocaliser targetMotionLocaliser,
		...
		resultTable = new PamTable(tableDataModel);
...
//	}
	...
	private class ResultTableDataModel extends AbstractTableModel {
		...
		private String[] colNames = {"Sel", "Model", "Symb", "Side", "Lat Long", "Depth", "Dist", "Error", "Chi2", "p", "nDF", "AIC", "millis","Comment"};
		@Override
		...
		public Object getValueAt(int row, int col) {
			ArrayList<TargetMotionResult> results = targetMotionLocaliser.getResults();
			TargetMotionResult aResult = results.get(row);
			Double a;
			Integer intVal;
			switch(col) {
			case 0:
			...
			case 2:
				PamSymbol aSymbol = aResult.getModel().getPlotSymbol(aResult.getSide());
				if (aSymbol == null) {
					return null;
				}
				return aSymbol;
...
//				aResult.get
		...
		public void setValueAt(Object aValue, int row, int col) {
			...
			if (col == 13) {
				targetMotionLocaliser.getResults().get(row).setComment(aValue.toString());
			}
			else {
