...
public class ByteConverterAifInt32 extends ByteConverter {
		...
		for (iChan = 0; iChan < nChan; iChan++) {
			...
			for (iSamp = 0; iSamp < nSamples; iSamp++) {
				doubleData[iChan][iSamp] = ((byteData[iPos]&0xFF)<<24 |
						...
						(byteData[iPos+2]&0xFF)<<8 |
						(byteData[iPos+3]&0xFF))/scale;
				iPos += blockSize;
			}
		}
		}
	...
	public int doubleToBytes(double[][] doubleData, byte[] byteData, int numSamples) {
		...
		for (iChan = 0; iChan < nChan; iChan++) {
			...
			for (iSamp = 0; iSamp < numSamples; iSamp++) {
				...
				byteData[iPos+3] = (byte) (intVal & 0xFF);
				byteData[iPos+2] = (byte) ((intVal & 0xFF00) >> 8);
				byteData[iPos+1] = (byte) ((intVal & 0xFF0000) >> 16);
				byteData[iPos] = (byte) (intVal>>24);
				iPos += blockSize;
			}
		}
