...
public class ByteConverterWavInt24 extends ByteConverter {

	double scaleBytesToDouble = Math.pow(2, 31);
	double scaleDoubleToByte = Math.pow(2, 23);
...
//		double scale = 1<<31;
		...
		for (iChan = 0; iChan < nChan; iChan++) {
			...
			for (iSamp = 0; iSamp < nSamples; iSamp++) {
				doubleData[iChan][iSamp] = ((byteData[iPos+2]&0xFF)<<24 |
						(byteData[iPos+1]&0xFF)<<16 |
						(byteData[iPos]&0xFF)<<8)/scaleBytesToDouble;
				iPos += blockSize;
			}
		}
		}
	...
	public int doubleToBytes(double[][] doubleData, byte[] byteData, int numSamples) {
		...
		for (iChan = 0; iChan < nChan; iChan++) {
			...
			for (iSamp = 0; iSamp < numSamples; iSamp++) {
				intVal = (int)(doubleData[iChan][iSamp]*scaleDoubleToByte);
				byteData[iPos] = (byte) (intVal & 0xFF);
				byteData[iPos+1] = (byte) ((intVal & 0xFF00) >> 8);
				byteData[iPos+2] = (byte) (intVal>>16);
				iPos += blockSize;
			}
		}
