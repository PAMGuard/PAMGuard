...
public class PamTabbedPane extends JTabbedPane implements ColorManaged {
	...
	private class TabPaneMouse extends MouseAdapter {

		@Override
		public void mouseClicked(MouseEvent e) {
			if (e.getClickCount() == 2 && pamGui != null) {
				pamGui.toggleFullScreen();
			}
		}
	...
	protected void processMouseEvent(MouseEvent evt){
		if (evt.getID() == MouseEvent.MOUSE_PRESSED && evt.getButton() == MouseEvent.BUTTON3){
			...
			if (index >= 0) {
				pamControllerInterface.getGuiFrameManager().getTabPopupMenu(pamGui, index).show(this, evt.getX(), evt.getY());
			}
		}else{
	...
	private int getTargetTabIndex(Point glassPt) {
		Point tabPt = SwingUtilities.convertPoint(glassPane, glassPt, PamTabbedPane.this);
