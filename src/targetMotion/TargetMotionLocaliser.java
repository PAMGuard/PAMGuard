...
import targetMotion.algorithms.LeastSquaresNew;
import targetMotion.algorithms.Simplex2DNew;
import targetMotion.algorithms.Simplex3DNew;
import targetMotion.dialog.TargetMotionDialog;
import targetMotion.tethys.TMALocalizationCreator;
import targetMotion.tethys.TMAOptionsPanel;
 ...
 * @param <T>
 */
public class TargetMotionLocaliser<T extends DetectionGroup> extends AbstractLocaliser<T> implements LocalisationAlgorithm, LocalisationAlgorithmInfo, PamSettings  {
	...
	/**
	 * Observer instance so that it can be moved between data blocks when the
	 * localiser is pointed at a different type of detection group.
	 */
	...
	public TargetMotionLocaliser(PamControlledUnit pamControlledUnit, PamDataBlock<T> dataBlock, PamDataBlock subDetectionBlock) {
		...
		dataBlock.addObserver(dataObserver = new DataObserver(), false);
...
//		models.add(new Simplex3D<T>(this));
	...
	public boolean showTMDialog(T dataUnit) {
		if (dataUnit != null && dataUnit.getParentDataBlock() != null
				&& dataUnit.getParentDataBlock() != dataBlock) {
			// point the localiser at whatever type of detection group this is
			// (click train, CPOD train, detection group, ...).
			setEventDataBlock((PamDataBlock<T>) dataUnit.getParentDataBlock());
		}
	...
	/**
	 * Change the data block of detection groups the localiser is working with.
	 * The target motion dialog's event list will show the contents of this
	 * block. Allows the localiser to work with any type of detection group
	 * (manually annotated events, click trains, ...).
	 * @param newDataBlock the data block to work with.
	 */
	public void setEventDataBlock(PamDataBlock<T> newDataBlock) {
		if (newDataBlock == null || newDataBlock == dataBlock) {
			return;
		}
		if (dataBlock != null && dataObserver != null) {
			dataBlock.deleteObserver(dataObserver);
		}
		dataBlock = newDataBlock;
		setDataBlock(newDataBlock);
		newDataBlock.addObserver(dataObserver, false);
		clearResults();
		if (targetMotionDialog != null) {
			targetMotionDialog.updateEventList();
		}
