...
import targetMotion.TargetMotionLocalisation;
import targetMotion.TargetMotionLocaliser;
import targetMotion.TargetMotionLocaliser.Interractive;
 ...
 * @param <T>
 */
public class EventControlPanel<T extends DetectionGroup> implements TMDialogComponent{
...
//		allEvents.setEnabled(eventLocalisationProgress.progressType == EventLocalisationProgress.DONE);
	...
	void updateEventList() {
		// the localiser can be re-pointed at a different detection group data
		// block, so always take the current one.
		dataBlock = targetMotionLocaliser.getDataBlock();
		if (mainPanel.getBorder() instanceof TitledBorder) {
			((TitledBorder) mainPanel.getBorder()).setTitle(dataBlock.getDataName());
			mainPanel.repaint();
		}
