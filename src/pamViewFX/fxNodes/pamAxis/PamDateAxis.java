...
public class PamDateAxis extends ValueAxis<Long> {
	...
	public PamDateAxis() {
		...
		//make sure that the tick marks update whenever the axis is manually set. 
		this.upperBoundProperty().addListener((obsval, oldVal, newVal)->{
			setTickDateFormatter();
		});

		this.lowerBoundProperty().addListener((obsval, oldVal, newVal)->{
			setTickDateFormatter();
		});
	}

	private void setTickDateFormatter() {
		double labelSize = getTickLabelFont().getSize() * 2;
		double[] rangeProps =  (double[]) autoRange(getLowerBound(),getUpperBound(),getWidth(),labelSize);
		final double rangeIndex = rangeProps[4];
		currentRangeIndexProperty.set((int)rangeIndex);
		//    	System.out.println("Set data formatter: " + currentRangeIndexProperty.get());
	...
	@Override protected Object autoRange(double minValue, double maxValue, double length, double labelSize) {
		...
		while (reqLength > length || count > 20) {
			...
			for (int i=0; i<TICK_UNIT_DEFAULTS.length; i++) {
				...
				if (tickUnitDefault > tickUnit) {
					tickUnitRounded = tickUnitDefault;
					//                    System.out.println("Found range index: " + i + " " + tickUnit); 
					rangeIndex = i;
					break;
				}
			}
