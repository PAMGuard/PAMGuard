 ...
 *
 */
public class EventRotator {

	private SuperDetection pamDetection;
	...
	/**
	 * Array angles for each sub detection - radians anticlockwise from x axis. ). 
	 */
...
//					Math.toDegrees(rotatedArrayAngles[i]), subDetectionHeadings[i].toString()));
	...
	private void calculateWorldVectors() {		
		worldVectors = new PamVector[nSubDetections][];
		PamVector[] v;
		PamDataUnit pd;
		AbstractLocalisation localisation;
		for (int i = 0; i < nSubDetections; i++) {
