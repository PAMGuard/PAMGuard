 ...
 * @param <T>
 */
public class DialogMap2D<T extends GroupDetection> extends DialogMap<T> implements ColorManaged {
	 ...
	 */
	public DialogMap2D(TargetMotionLocaliser<T> targetMotionLocaliser,
...
//		}
	...
	private void sortPlotDimensions() {
		...
		int nSubDetections=eventRotator.getnSubDetections();
		if (nSubDetections == 0) {
			return;
		}
		PamDataUnit pd = currentEvent.getSubDetection(0);
		if (pd == null) {
			return;
		}
		...
		if (rotated) {
			originVectors = eventRotator.getRotatedOrigins();
			arrayHeadingVectors = eventRotator.getRotatedHeadings();
			worldVectors = eventRotator.getRotatedWorldVectors();
		}
		else {
			originVectors = eventRotator.getSubDetectionOrigins();
			arrayHeadingVectors = eventRotator.getSubDetectionHeadings();
			worldVectors = eventRotator.getWorldVectors();
		}
	...
	Point latLongToPoint(LatLong ll) {
		...
		PamVector v = eventRotator.latLongToMetres(ll, rotated);
	...
	LatLong pointToLatLong(Point pt) {
		...
		return eventRotator.metresToLatLong(v, rotated);
	}
...
//			System.out.println(String.format("Key %d mods %d are %s", e.getKeyCode(), mods, txt));
	...
	class MapAxisPanel extends PamAxisPanel implements ColorManaged{
...
//			this.setEnabled(true);
		...
		public String getToolTipText(MouseEvent event) {
			...
			GroupLocResult nearResult = findNearResult(pt);
			if (nearResult == null) {
...
//			synchronized (targetMotionLocaliser.getDataSynchObject()) {
				...
				if (currentEvent == lastEvent) {
					if (currentEvent.getLastUpdateTime() > lastUpdateTime) {
						sortPlotDimensions();
				...
				try {
					plotGpsTrack(g, currentEvent);

					plotSubDetections(g, currentEvent);

					plotResults(g, currentEvent);
					
					plotErrors(g, currentEvent);
				}
				catch (Exception e) {
//					e.printStackTrace();
				}
//			}
		 ...
		 */
		private GroupLocResult findNearResult(Point point) {
			...
			int n = results.size();
			LatLong ll;
			GroupLocResult aResult;
			GroupLocResult nearResult = null;
			Point pt;
			int nearDist = 100;
			int newDist;
			for (int i = 0; i < n; i++) {
		...
		private void plotResults(Graphics g, T currentEvent) {
			ArrayList<GroupLocResult> results = targetMotionLocaliser.getResults();
			...
			for (int i = 0; i < n; i++) {
				aResult = results.get(i);
				aSymbol =((TargetMotionModel) aResult.getModel()).getPlotSymbol(aResult.getSide());
				if (aSymbol == null) {
		...
		private void plotErrors(Graphics g, T currentEvent) {
			// TODO Auto-generated method stub
			
		}


		private void plotGpsTrack(Graphics g, T currentEvent) {
			...
			long startTime = 0;
			if (currentEvent != null) {
				startTime = currentEvent.getTimeMilliseconds();
			}
...
//			}
		...
		private void plotSubDetections(Graphics g, T currentEvent) {
			...
			for (int i = 0; i < nSubDetections; i++) {	
				 ...
				 */
				localisation = currentEvent.getSubDetection(i).getLocalisation();
